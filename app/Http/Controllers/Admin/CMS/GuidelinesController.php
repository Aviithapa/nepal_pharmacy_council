...
class GuidelinesController extends Controller
{
    ...
    {
        $publication = $this->postRepository->getPaginatedList($request, 'guidelines');
        return view('admin.pages.cms.guidelines.index', compact('publication', 'request'));
    }
    ...
    public function store(CreateBannerRequest $request)
    {
        ...
        try {
            ...
            if (isset($data['file'])) {
                $response =  $this->fileUploader->upload($data['file'], "guidelines");
            ...
            DB::commit();
            session()->flash('success', 'Guidelines has been created successfully.');
    ...
    public function destroy(string $id)
    {
        try {
            ...
            session()->flash('success', 'Guidelines been deleted successfully.');
            return redirect()->route('guidelines.index');
        } catch (Exception $e) {
            session()->flash('danger', 'Oops! Something went wrong.' . $e);
            return redirect()->back()->withInput();
