...
class CodeOfConductController extends Controller
{
    ...
    {
        $publication = $this->postRepository->getPaginatedList($request, 'code-of-conduct');
        return view('admin.pages.cms.coc.index', compact('publication', 'request'));
    }
    ...
    public function store(CreateBannerRequest $request)
    {
        ...
        try {
            ...
            if (isset($data['file'])) {
                $response =  $this->fileUploader->upload($data['file'], "code-of-conduct");
            ...
            DB::commit();
            session()->flash('success', 'CodeOfConduct has been created successfully.');
    ...
    public function destroy(string $id)
    {
        try {
            ...
            session()->flash('success', 'CodeOfConduct been deleted successfully.');
            return redirect()->route('coc.index');
        } catch (Exception $e) {
            session()->flash('danger', 'Oops! Something went wrong.' . $e);
            return redirect()->back()->withInput();
